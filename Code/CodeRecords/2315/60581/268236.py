...
def getNextLever(thisLever):
    nextLever = []
    for i in range(len(thisLever)):
        for j in range(len(connect)):
            if connect[j][0] == thisLever[i]:
                nextLever.append(connect[j][1])
    return nextLever

while getNextLever(thisLever) != []:
    count += 1
    thisLever = getNextLever(thisLever)
