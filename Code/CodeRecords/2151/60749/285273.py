...
struct data {
    int x, y;
} b[3], a[N][N], res[3], w[3], E[3][3], Uni = { 1, 0 }, Inv3;
...
data operator*(const data& a, const data& b) {
    return (data){ (1ll * a.x * b.x % mod + mod - 3ll * a.y * b.y % mod) % mod,
                   (1ll * a.x * b.y + 1ll * a.y * b.x) % mod };
}
...
int main() {
    ...
    Inv3 = (data){ fastpow(3, mod - 2), 0 };
    w[0] = (data){ 1, 0 };
    w[1] = (data){ mod - Inv2, Inv2 };
    w[2] = (data){ mod - Inv2, mod - Inv2 };
    for (int i = 0; i < 3; i++) {
        E[i][i] = (data){ 1, 0 };
        DFT(E[i]);
    }
