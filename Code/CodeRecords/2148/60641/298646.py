def main():
    ...
    matrix = [[True for a in range(0, num_of_debugs + 1)] for b in range(0, 2 ** num_of_bugs + 1)]
    for i in range(0, 2 ** num_of_bugs):
        for j in range(0, num_of_debugs):
            if matrix[i + 1][j + 1]:
                result = find_edge(i, solutions[j], graph)
                if result is not None:
                    matrix[result][j + 1] = False
