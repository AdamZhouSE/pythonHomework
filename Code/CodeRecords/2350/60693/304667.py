...
void DFS(int x,int S,int tot)
{
    ...
    if(x)DFS(x+1,S,tot+std::min(c[x]-s,b[x]-c[x]+s));
}

int main()
{
    for(int i=1;i<1<<22;++i)Cnt[i]=Cnt[i>>1]+(i&1);
    for(scanf("%d",&T);T--;)
    {
        scanf("%d",&n),m=0,Ans=1e9;
        for(int i=1;i<=n;++i)scanf("%d",&a[i]),Last[i]=b[i]=c[i]=st[i]=0;
        for(int i=n;i>=1;--i)
            if(!Last[a[i]])Last[a[i]]=i;
            else l[m]=i,r[m]=Last[a[i]],++m;
