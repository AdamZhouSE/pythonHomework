...
void prt(int o){ int x=p[o];for(printf("%d ",o);x!=o;x=p[x]) printf("%d ",x); }
void init(){
	sort(c+1,c+1+n);
...
int main(){
	...
	fr(i,1,n)
	 if (a[i]!=c[i]&&!b[i]){
	 	...
	 	while(!b[x]&&c[use[a[x]]]==a[x]){
	 		b[x]=1;
	 		int &w=use[a[x]];while(c[w]==a[w]&&c[w]==a[x]) w++;
	 		if (c[w]!=a[x]) break;
			unio(x,w);p[x]=w;
			x=w++;
		}
	...
	fr(i,1,n)
	 if (a[i]!=c[i]){
	 	if (!b[a[i]]) b[a[i]]=i;
	  		...
	  		if (fx!=fy) swap(p[x],p[i]),unio(fx,fy);
	  	 }
	 }
	...
	if (cnt<=1||s<=2){
		...
		fr(i,1,s) sum+=sz[v[i]];
		printf("%d\n",sum);
		fr(i,1,s) prt(v[i]);puts("");
		printf("%d\n",s);
		rf(i,s,1) printf("%d ",v[i]);puts("");
