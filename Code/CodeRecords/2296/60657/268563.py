...
if __name__=='__main__':
    ...
    for i in range(int(b[0])):
        ...
        tempval=temp[3]
        tree.father_son_insert_val(temproot,templeft,tempright,tempval)
    ...
    for k in r:
            ...
            for j in sonsub:
                tempsum=[]
                for h in j:
                    tempsum.append(int(tree.search_node(tree.root,h).val))
                cons.append((sum(tempsum),len(j)))
    final=[]
    for i in cons:
        if i[0]==want:
            final.append(i[1])
