...
for i in range(0, num_of_sentence):
    ...
    if information[0] == "M":
        ...
        information_of_children = sorted(information_of_children, key=lambda x: (x[0], x[1]), reverse=True)
        result = -1
        for j in range(0, len(information_of_children)):
            if information_of_children[j][0] <= int(information[1]):
                temp = sorted(information_of_children[j:],key=lambda x:x[1],reverse=True)
