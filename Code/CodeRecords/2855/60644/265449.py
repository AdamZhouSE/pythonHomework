        ...
        for j in range(0, n):
            if i == 0:
                ...
                if j == 0:
                    ...
                    if (arr[n - 1][j - 1].count('o') + arr[n - 2][j].count('o') + arr[n - 1][j + 1].count(
                            'o')) % 2 == 1:
                        res = False
                        break
            else:
                if j == 0:
                    ...
                    if (arr[i][n - 2].count('o') + arr[i - 1][n - 1].count('o') + arr[i + 1][n - 1].count(
                            'o')) % 2 == 1:
                        res = False
                        break
                else:
                    if (arr[i][j - 1].count('o') + arr[i - 1][j].count('o') + arr[i][j + 1].count('o') + arr[i + 1][
                        j].count('o')) % 2 == 1:
                        res = False
                        break
if n==1:
