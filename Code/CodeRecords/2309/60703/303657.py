...
for i in range(row):
    ...
    for j in range(col):
        if li[j] == '*':
            ...
            add_edge(source, mark_of_node(i, j), 1)
            if i > 0 and previousline[j] == "3":
                add_edge(mark_of_node(i, j), mark_of_node(i - 1, j), 1)
            if j > 0 and li[j - 1] == "3":
                add_edge(mark_of_node(i, j), mark_of_node(i, j - 1), 1)
            if j < col - 1 and li[j + 1] == "3":
                add_edge(mark_of_node(i, j), mark_of_node(i, j + 1), 1)
        elif li[j] == "3":
            add_edge(mark_of_node(i, j), sink, 1)
            if i > 0 and previousline[j] == "1":
                add_edge(mark_of_node(i - 1, j), mark_of_node(i, j), 1)
    previousline = li
# print(graph)
