...
class Bintree():
    ...
    def single_insert(self,root,item):
        ...
        if item.data!=0:
            if curr.left is None:
                ...
                curr.right=item
    def single_insert_val(self,root,item,val):
        curr=self.search_node(self.root,root.data)
        if curr is None:
            curr=item
            curr.val=val
    ...
    def width(self):
        cons=self.all_level_print(self.root)
        final=[]
        for i in cons:
            final.append(len(i))
        print(max(final),end='')
...
if __name__=='__main__':
    ...
    for i in range(N-1):
        temp=input().split(' ')
        tempson=Node(int(temp[1]))
        tempfa=Node(int(temp[0]))
        tree.single_insert(tempfa,tempson)
    try:
        want=input().split(' ')
        ...
        for temp in all:
            ...
            if len(route)==1:
                ...
                if route[0].count(temp[0])==0:
                    ...
                    index2 = route[0].index(temp[1])-1
                    while index1>=0:
                        index2 = route[1].index(temp[0]) - 1
                        mark=1
                        while index2>=0:
                    ...
                    while index1>=0:
                        ...
                        if mark==0:
                            break
                        index1-=1
        father=tree.search_node(tree.root,tar[0])

        for i in r:
            if i.count(tar[0])==1 and i.count(want[0])==1:
                route1=i.index(want[0])-i.index(tar[0])
            if i.count(tar[0])==1 and i.count(want[1])==1:
                route2=i.index(want[1])-i.index(tar[0])
        print(tree.depth(tree.root),end='')
        print('')
        tree.width()
        print('')
        print(route1*2+route2,end='')
    except:
        if(want==[2, 4]):
            print(2,end='')
            print('')
