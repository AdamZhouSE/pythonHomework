...
def find(source,i,j,alls):
    if i==0 and j==0:
        ...
        if(not location(source,min([source[0][j-1],source[1][j]])) in alls):
            return min([source[0][j-1],source[1,j]])
    elif j==0 and i==len(source)-1:
        if(not location(source,min([source[i-1][0],source[i][1]])) in alls):
            return min([source[i-1][0],source[i][1]])
    elif i==0:
        if(not location(source,min([source[i][j-1],source[i][j+1],source[i+1][j]])) in alls):
            return min([source[i][j-1],source[i][j+1],source[i+1][j]])
    elif j==0:
        if(not location(source,min([source[i-1][j],source[i+1][j],source[i][j+1]])) in alls):
            return min([source[i-1][j],source[i+1][j],source[i][j+1]])
    elif i==len(source)-1:
        if(not location(source,min([source[i-1][j],source[i][j-1],source[i][j+1]])) in alls):
            return min([source[i-1][j],source[i][j-1],source[i][j+1]])
    elif j==len(source[0])-1:
        if(not location(source,min([source[i-1][j],source[i+1][j],source[i][j-1]])) in alls):
            return min([source[i-1][j],source[i+1][j],source[i][j-1]])
    else:
        if(not location(source,min([source[i-1][j],source[i+1][j],source[i][j-1],source[i][j+1]])) in alls):
