    ...
    for edge in tree_edges:
        if s > 0:
            if s >= 2:
                ...
                if edge[0] in vertices and edges[1] not in vertices:
                    vertices.append(edge[1])
                    s -= 1
                    tree_edges.remove(edge)
                elif edges[1] in vertices and edges[2] not in vertices:
                    vertices.append(edge[0])
                    s -= 1
                    tree_edges.remove(edge)
        else:
            break
    if tree_edges[0][2] == 212.13 and temp != 2:
        print(temp)
        print(xs)
        print(ys)
    print(tree_edges[0][2], end="")
