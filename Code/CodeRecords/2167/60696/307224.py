...
void change(int &rt, int l, int r, int x, int val) {
    ...
    int mid = (l + r) / 2;
    if (x <= mid)
        change(L[rt], l, mid, x, val);
    else
        change(R[rt], mid + 1, r, x, val);
...
int query1(int rt, int l, int r, int x) {
    ...
    int mid = (l + r) / 2;
    if (x <= mid)
        return query1(L[rt], l, mid, x);
    else
        return tree[L[rt]] + query1(R[rt], mid + 1, r, x);
}
int query2(int rt, int l, int r, int x) {
    ...
    int mid = (l + r) / 2;
    if (x <= mid)
        return tree[R[rt]] + query2(L[rt], l, mid, x);
    else
        return query2(R[rt], mid + 1, r, x);
}
int find(int x) {
    if (fa[x] == x)
        return x;
