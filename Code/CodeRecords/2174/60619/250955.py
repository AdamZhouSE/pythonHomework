...
for i in range(islandsNum):
    ...
    for j in range(islandsNum):
        p.append(maxN)
    theMap.append(p)
for index in range(insNum):
    instruction = input().split(" ")
    if instruction[0] == "0":
        theMap[int(instruction[1]) - 1][int(instruction[2]) - 1] = theMap[int(instruction[2])-1][int(instruction[1])-1] = int(instruction[3])
    elif instruction[0] == "1":
        theMap[int(instruction[1]) - 1][int(instruction[2]) - 1] = theMap[int(instruction[2])-1][int(instruction[1])-1] = maxN
    else:
        start = int(instruction[1]) - 1
        end = int(instruction[2]) - 1
        print(find_short_path(islandsNum,start,end))
