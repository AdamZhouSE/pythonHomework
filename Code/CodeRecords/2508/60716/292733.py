def treeSides(node:int,index:int,apple:int):
    print("node: {}, index:{}, num:{}".format(node,index,apple))
#    print(node)
    ...
    if len(t)==0:return apple
        ...
        if index>=2:
            for i in range(index-2+1):
                ...
                print("no0 node:{} both with {}".format(node,apple + appa + appb))
        temp1 = apple+treeSides(tree[a][1],index-1,tree[a][2])
        print("no1 node:{} left with {}".format(node,temp1))
        temp2 = apple+treeSides(tree[b][1],index-1,tree[b][2])
        print("no2 node:{} left with {}".format(node,temp2))
        maxss.append(temp1)
        maxss.append(temp2)
        return max(maxss)
