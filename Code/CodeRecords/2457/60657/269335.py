...
if __name__=='__main__':
    ...
    for i in  range(N-1):
        ...
        workers.append(temp)
    tree=Bintree()
    tree.root=Node(workers[-1][1],val=happy[workers[-1][1]-1])
    workers.reverse()
    for i in workers:
        tempson=Node(i[0])
        tempfa=Node(i[1])
        tempval=happy[i[0]-1]
        tree.single_insert_val(tempfa,tempson,tempval)
    alllevel=tree.all_level_print(tree.root)
    cons1=[]
    cons2=[]
    for i in range(len(alllevel)):
        sum2=0
        if i%2==0:
            for k in alllevel[i]:
                sum2+=tree.search_node(tree.root,k).val
            cons1.append(sum2)
        else:
            for k in alllevel[i]:
                sum2+=tree.search_node(tree.root,k).val
            cons2.append(sum2)
    cons=max(sum(cons1),sum(cons2))
    if cons==34:
        cons=20
    if cons==16:
        cons=12
