...
ll dis[maxn],pre[maxn],down[maxn],sufmx[maxn][2],mx[2][2];pli s1[maxn],s2[maxn];
...
inline bool check(ll mid)
{
	...
	for(int i=1,j=1;i<=cnt;i++)
	{
		...
		for(int x=0;x<2;x++)
			for(int y=0;y<2;y++) mx[x][y]=max(mx[x][y],down[s2[i].second]+(x?-1:1)*pre[s2[i].second]+sufmx[j][y]+L-mid);
	}
	int p=1,q=cnt+1;
	for(int i=1;i<=cnt;i++)
	{
		...
		int j=max(p,q);
		if(j<=cnt&&pre[i]-pre[j]>=mx[0][1]&&-pre[i]-pre[j]>=mx[1][1]) return 1;
	}
	return 0;
}
...
int main()
{
	...
	while(1)
	{
		...
		ll l=0,r=dis[q[cnt]],mid,ans=1e18;clear();
		for(int i=1;i<=cnt;i++)
		{
			dis[q[i-1]]=dis[q[i+1]]=0;
			x=bfs(q[i]);down[i]=dis[x];clear();
			l=max(l,dis[bfs(x)]);clear();
		...
		sort(s1+1,s1+cnt+1);sort(s2+1,s2+cnt+1,cmp);
		sufmx[cnt+1][0]=sufmx[cnt+1][1]=-1e18;
		for(int i=cnt;i;i--)
			for(int j=0;j<2;j++) sufmx[i][j]=max(sufmx[i+1][j],down[s1[i].second]+(j?-1:1)*pre[s1[i].second]);
		while(l<=r)
		{
			mid=(l+r)>>1;
		...
		printf("%lld\n",ans);
 		for(int i=1;i<=n;i++) head[i]=0; tot=cnt=0;
	}
 	return 0;
}