...
def Tarjan(x,f,dfn,low,VT,en,nex,las,Cut):
    ...
    if f==0 and son>2:
        Cut[x]=True
def DFS(x,vis,Size,pbc,las,nex,vis,Cut,CTime,con):
    i=las[x]
    vis[x]=True
    Size[pbc]+=1
    while(i):
        ...
        if vis[y] or Cut[y]:
            if Cut[y] and Ctime[y]!=pbc:
                con[pbc]+=1
                Ctime[y]=pbc
        DFS(y,vis,Size,pbc,las,nex,vis,Cut,CTime,con)
n=10005
t=0
while(True):
    t+=1
    ...
    if(z==0):
        ...
        VT,tot,P,pbc,Ans_cut=0,0,0,0,0
        las=memset(0,n);
        dfn=memset(0,n);
        low=memset(0,n);
        Cut=memset(false,n);
        ...
        for i in range(1,P+1):
            if mark[i] and not vis[i] and not Cut[i]:
                pbc+=1
                DFS(i,vis,Size,pbc,las,nex,vis,Cut,CTime,con)
        for i in range(1,pbc+1):
            if con[i]==0:
                ...
                Ans_tot*=Size[i]
                Ans_cnt+=1
        print("Case "+str(t)+": "+str(Ans_cut)+" "+str(Ans_tot))