        ...
        for i in range(1, n+1):
            for k in range(1, q+1):
                ...
                if temp == 0 and dp[i][j] + dist[i][task[k-1].s] <= task[k-1].r:
                    dp[task[k-1].s][j + third[k]] = min(dp[task[k-1].s][j+third[k]], max(dp[i][j] + dist[i][task[k-1].s], task[k-1].l))
                if temp == 1 and dp[i][j] + dist[i][task[k-1].t] <= task[k-1].r:
                    dp[task[k-1].t][j+third[k]] = min(dp[task[k-1].t][j+third[k]], dp[i][j] + dist[i][task[k-1].t])
